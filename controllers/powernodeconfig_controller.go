...
func (r *PowerNodeConfigReconciler) applyPowerNodeConfig(
	...
	// PowerNodeConfigs (spec.powerProfile or spec.reservedCPUs[].powerProfile) or running pods.
	...
	if err := r.validatePowerNodeConfigProfiles(ctx, config, nodeName, logger); err != nil {
		...
		_ = r.updatePowerNodeStatusPools(ctx, nodeName, config.Name, config.Spec.PowerProfile, "", nil, []string{err.Error()}, logger)
	...
	if err := r.updatePowerNodeStatusPools(ctx, nodeName, config.Name, config.Spec.PowerProfile, sharedCPUIDs, reservedProfileCPUs, statusErrors, logger); err != nil {
		return ctrl.Result{}, err
	}
...
func (r *PowerNodeConfigReconciler) validatePowerNodeConfigProfiles(
	...
	if err := r.Get(ctx, client.ObjectKey{Name: config.Spec.PowerProfile, Namespace: PowerNamespace}, sharedProfile); err != nil {
		if errors.IsNotFound(err) {
			return fmt.Errorf("PowerProfile '%s' not found", config.Spec.PowerProfile)
		}
		return err
	}
	if !sharedProfile.Spec.Shared {
		return fmt.Errorf("PowerProfile '%s' is not a shared profile (spec.shared must be true)", config.Spec.PowerProfile)
	}

	// Validate all referenced profiles are available on this node.
	profileNames := []string{config.Spec.PowerProfile}
...
func (r *PowerNodeConfigReconciler) configureSharedPool(config *powerv1.PowerNodeConfig, logger *logr.Logger) error {
	pool := r.PowerLibrary.GetExclusivePool(config.Spec.PowerProfile)
	if pool == nil {
		return fmt.Errorf("pool for profile '%s' not found", config.Spec.PowerProfile)
	}
	profile := pool.GetPowerProfile()
	if profile == nil {
		return fmt.Errorf("profile for pool '%s' not found", config.Spec.PowerProfile)
	}
	...
	logger.V(5).Info("configured shared pool profile", "profile", config.Spec.PowerProfile)
	return nil
}
...
func (r *PowerNodeConfigReconciler) describeConflictingConfig(ctx context.Context, config *powerv1.PowerNodeConfig) string {
	...
	if err := r.Get(ctx, client.ObjectKey{Name: config.Spec.PowerProfile, Namespace: PowerNamespace}, profile); err != nil {
		if errors.IsNotFound(err) {
			return fmt.Sprintf("conflicting PowerNodeConfig: %s (profile '%s' not found)", config.Name, config.Spec.PowerProfile)
		}
		return fmt.Sprintf("conflicting PowerNodeConfig: %s", config.Name)
	}
	if !profile.Spec.Shared {
		return fmt.Sprintf("conflicting PowerNodeConfig: %s (profile '%s' is not a shared profile)", config.Name, config.Spec.PowerProfile)
	}
	return fmt.Sprintf("conflicting PowerNodeConfig: %s", config.Name)
}
...
func configReferencesProfile(config *powerv1.PowerNodeConfig, profileName string) bool {
	if config.Spec.PowerProfile == profileName {
		return true
	}
	for _, rc := range config.Spec.ReservedCPUs {
